...
function adding(collectionMe, docMe, data, merge){
    if(merge==true){
        db.collection(collectionMe).doc(docMe).add(data, { merge: true });

    }
    if(merge==false){
        db.collection(collectionMe).doc(docMe).set(data, {merge: false});
