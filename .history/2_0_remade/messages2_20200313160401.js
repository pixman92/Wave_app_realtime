...
async function one(adminEmail){
    ...
    try{
        ...
        tmp = db.collection('chatrooms3/');

        tmp2 = tmp.where('adminEmail', '==', memberEmail)

        tmp2.get().then(async (snap)=>snap.forEach(async (doc)=>{
            console.log(doc)
            await tmpId.push(doc.id);
        }));
...
async function two(etc, memberEmail){
    ...
    try{
        ...
        tmp = db.collection('chatrooms3/' + tmpId[0] + '/messagesData/');

        tmp2 = tmp.where('memberEmail', '==', memberEmail)
